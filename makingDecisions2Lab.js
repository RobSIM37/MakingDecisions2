...
if (score >= 90 && score <=100){
  ...
  myLetterGrade = "Invalid Data Entry";
}
...
if (myFavoriteNumbers.length>=targetPosition){
  someNum = myFavoriteNumbers[targetPosition-1];
} else {
...
function FizzBuzz(numbers, words, countTo){
    ...
    for (let currentNumber = 1; currentNumber <= countTo; currentNumber++){ //start counting from one and go to the countTo value
        ...
        let output = ""; //create a value to be logged later

        for (let i = 0; i < numbers.length; i++){// iterate over each number in the numbers array

            // if the current number is divisible by element in the numbers array and there is a corresponding word in the words array
            if (currentNumber % numbers[i] === 0 && i<words.length){

                output = `${output}${words[i]}`; //Add the new word to the end of the output
